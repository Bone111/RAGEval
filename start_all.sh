...
if [ "$MODE" = "init" ]; then
  ...
  echo "启动数据库..."
  cd rag-evaluation-backend && docker-compose up -d
  cd ..
  echo "启动后端..."
  kill_port 8000
  ...
  echo "启动前端..."
  kill_port 5175
  cd rag-evaluation-frontend && npm run dev -- --port 5175 &
  cd ..
  echo "全部服务已启动。"
  echo "访问前端: http://localhost:5175"
  echo "访问后端: http://localhost:8000/docs"
  exit 0
fi
